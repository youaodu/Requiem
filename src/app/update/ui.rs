...
impl Requiem {
    ...
    pub fn handle_ai_fill_input_action(&mut self, action: iced::widget::text_editor::Action) -> Task<Message> {
            ...
            AiEngine::OpenAI => {
                ...
                if api_key.trim().is_empty() {
                    ...
                    return Task::done(Message::ShowToast(
                        crate::ui::toast::Toast::error("OpenAI API Key is not configured".to_string())
                    ));
                }
                ...
                Task::perform(
                    async move {
                        crate::http_client::call_openai_api(&api_url, &api_key, &model, &user_prompt).await
                    },
            ...
            Ok(generated_response) => {
                ...
                let ai_request: AiGeneratedRequest = match serde_json::from_str(&json_str) {
                    ...
                    Err(e) => {
                        ...
                        return Task::done(Message::ShowToast(
                            crate::ui::toast::Toast::error(format!("Failed to parse AI response: {}", e))
                        ));
                    }
                };
                ...
                if let Some(current_request) = self.get_current_request_mut() {
                    ...
                    if let Ok(method) = ai_request.method.to_uppercase().parse::<crate::models::HttpMethod>() {
                    ...
                    current_request.headers = ai_request.headers
                        .into_iter()
                        .map(|h| crate::models::KeyValue::new(h.key, h.value))
                        .collect();

                    // Set query params
                    current_request.query_params = ai_request.params
                        .into_iter()
                    ...
                    if !ai_request.body.is_empty() {
                        current_request.body = crate::models::BodyType::Json(ai_request.body.clone());

                        // Update the request body content editor
                        self.request_body_content = iced::widget::text_editor::Content::with_text(&ai_request.body);
                    }

                    Task::done(Message::ShowToast(
                        crate::ui::toast::Toast::success("AI Fill completed successfully".to_string())
                    ))
                } else {
                    Task::done(Message::ShowToast(
                        crate::ui::toast::Toast::warning("No active request to fill".to_string())
                    ))
                }
            }
            Err(error) => {
                ...
                Task::done(Message::ShowToast(
                    crate::ui::toast::Toast::error(format!("AI Fill failed: {}", error))
                ))
            }
        }
    }
