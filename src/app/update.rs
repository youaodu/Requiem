...
impl Requiem {
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            Message::AiEngineChanged(engine) => self.handle_ai_engine_changed(engine),
            Message::AiApiUrlChanged(url) => self.handle_ai_api_url_changed(url),
            Message::AiApiKeyChanged(key) => self.handle_ai_api_key_changed(key),
            Message::AiModelChanged(model) => self.handle_ai_model_changed(model),
            Message::ShowAiFillDialog => self.handle_show_ai_fill_dialog(),
            Message::CloseAiFillDialog => self.handle_close_ai_fill_dialog(),
            Message::AiFillInputAction(action) => self.handle_ai_fill_input_action(action),
            Message::ConfirmAiFill => self.handle_confirm_ai_fill(),
